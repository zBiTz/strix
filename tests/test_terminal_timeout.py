...
class TestTerminalTimeout:
    ...
    def test_simple_command_completes_quickly(self, terminal_manager: TerminalManager) -> None:
    ...
    def test_chained_commands_complete_quickly(self, terminal_manager: TerminalManager) -> None:
    ...
    def test_timeout_triggers_properly(self, terminal_manager: TerminalManager) -> None:
        ...
        assert (
            "still running" in result["content"].lower() or "timeout" in result["content"].lower()
        )

    def test_max_command_timeout_enforced(self) -> None:
        """Test that MAX_COMMAND_TIMEOUT constant is defined."""
        # Check that the constant exists and has expected value
        assert hasattr(TerminalSession, "MAX_COMMAND_TIMEOUT")
        assert TerminalSession.MAX_COMMAND_TIMEOUT == 300.0

    def test_hard_timeout_protection(self, terminal_manager: TerminalManager) -> None:
...
class TestTerminalPromptDetection:
    ...
    def test_detect_command_completion_exists(self, terminal_session: TerminalSession) -> None:
        """Test that fallback detection method exists."""
        # Verify the method exists
        assert hasattr(terminal_session, "_detect_command_completion")

    def test_try_recover_session_exists(self, terminal_session: TerminalSession) -> None:
        """Test that session recovery method exists."""
        # Verify the method exists
        assert hasattr(terminal_session, "_try_recover_session")

    def test_verify_prompt_setup_exists(self, terminal_session: TerminalSession) -> None:
